...
function getRobberyCount(alexa, neighborhood, month, skillName) {
    var robberyCount = -1;
    ...
    step (
        function getRobberyCount() {
            var sql = " select count(CRIME_TYPE) as cnt " +
                      ...
                      " and ZIP_CODE in (select zip from zipCodeLookup where neighborhood = ? ) and ";
                ...
                if(robberyCount == 0) {
                    step (
                        ...
                        function finish(err, rows, fields) {
                            ...
                            //connection.end();
                            alexa.emit(':tellWithCard', speechOutput, skillName, speechOutput);
                        }
                    );
                } else {
                    speechOutput = buildSpeechOutput(robberyCount, neighborhood, month);
...
module.exports.getRobberyCount = getRobberyCount;
